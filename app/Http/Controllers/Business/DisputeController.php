...
use App\Models\CreditorLetter;
use App\Models\CreditorLetterRecord;
use App\Models\Dispute;
use App\Models\DisputeLetter;
use App\Models\DisputeLetterRecord;
...
class DisputeController extends Controller
{
    ...
    public function getDisputeStatuses(){
        $page = 'Disputes Status';
        $disputes = Dispute::where('owner_id', auth()->user()->id)->orderBy('id','desc')->get();
        return view('business.disputes.index', compact('page','disputes'));

    }

    public function getDisputes(){
        $page = 'Disputes';
        $letters = DisputeLetter::where('owner_id', auth()->user()->id)->orderBy('id','desc')->get();
//        dd($letters);
//        $collection = new \Illuminate\Support\Collection();
//        $letters = $collection->merge($letters)->merge($creditor_letters)->sortByDesc('created_at');
        return view('business.disputes.letters', compact('page','letters'));
    ...
    public function getDisputeLetter($id){
        ...
        if(!is_null($letter)){
            ...
            return redirect()->route('business.disputes')->with('error', 'No letter found.');
        }
    }
    ...
    public function editDisputeLetter($id){
        ...
        $letter = DisputeLetter::where('id', $id)->where('owner_id', auth()->user()->id)->first();
        if(!is_null($letter)){
            return view('business.disputes.editor', compact('page','letter'));
        }else{
            return redirect()->route('business.disputes')->with('error', 'No letter found.');
        }
    }
    ...
    public function editDisputeLetterPost(Request  $request, $id){
        ...
        if(!is_null($letter)){
            ...
            if(strtolower($request->submitButton) == 'pdf'){
                ...
                return redirect()->route('business.disputes')->with('error', 'Something went wrong, please try later.');
            }
    ...
    public function faxDisputeLetter(Request  $request,$id){
        $page = 'Disputes';
        $letter = DisputeLetter::where('id', $id)->where('owner_id', auth()->user()->id)->where('fax_sent',0)->first();
        if(!is_null($letter)){
            $letter->fax_sent = 1;
            $letter->save();
    ...
    public function createDispute (Request $request, $id, $type){
...
//                }
                DB::commit();
                return redirect()->route('business.disputes')->with('success', 'Dispute created successfully.');
            }catch (\Exception $e) {
                dd($e->getMessage());
                DB::rollBack();
            }
        ...
        dd($request->get('bureau'));
        return redirect()->route('business.index')->with('success', "Dispute created successfully.");
    }

    public function createDisputeUpdate (Request $request, $id, $type){
        $client = User::findorfail($id);
        $validator = Validator::make($request->all(), [
            'bureau' => 'required|array',
            'creditor.*' => 'required_with:bureau.*',
        ...
        if(count($request->get('bureau')) > 0){
            ...
            try {
                ...
                DB::commit();
                return redirect()->route('business.get.disputes.editor', $tracking_id)->with('message', 'Dispute created successfully.');
            }catch (\Exception $e) {
                DB::rollBack();
    ...
    public function testDispute(Request $request){
        $jsonString = file_get_contents(base_path('public/assets/business/sampleData.json'));
...
//        dd($credit_score);
    ...
    public function testLetters(){
        $tu_diputes = Dispute::where('uid','6128717b9ea58')->where('is_tu',1)->orderBy('id','asc')->get();
        $tu_reasons = [];
        foreach ($tu_diputes as $dispute){
            if(array_key_exists($dispute->reason_id,$tu_reasons)){
                $tu_reasons[$dispute->reason_id] [] = $dispute->id;
        ...
        $exp_diputes = Dispute::where('uid','61287e33a7ba1')->where('is_exp',1)->orderBy('id','asc')->get();
        $exp_reasons = [];
        foreach ($exp_diputes as $dispute){
            if(array_key_exists($dispute->reason_id,$exp_reasons)){
                $exp_reasons[$dispute->reason_id] [] = $dispute->id;
        ...
        $eqfx_diputes = Dispute::where('uid','6128717b9ea58')->where('is_eqfx',1)->orderBy('id','asc')->get();
        $eqfx_reasons = [];
        foreach ($eqfx_diputes as $dispute){
            if(array_key_exists($dispute->reason_id,$eqfx_reasons)){
                $eqfx_reasons[$dispute->reason_id] [] = $dispute->id;
        ...
        $client_info = ucwords($client->first_name).' '.$client->last_name .' <br> ' .$client->mailing_address;
        foreach ($exp_reasons as $exp_reason=>$values){
//            $exp_reason_letter  = DisputeLetter::where('dispute_uid','6128717b9ea58')->where('reason_id', $exp_reason)->where('bureau_id',2)->first();
//            if(is_null($exp_reason_letter)){
//                $exp_reason_letter = new DisputeLetter();
//                $exp_reason_letter->dispute_uid = '6128717b9ea58';
//                $exp_reason_letter->reason_id = $exp_reason;
//                $exp_reason_letter->bureau_id = 2;
//                $exp_reason_letter->bureau = 'Experian';
//                $exp_reason_letter->save();
//
//            }

            $bureau_info = 'Experian Information Services LLC';
            $letter_og = Letter::find($exp_reason);
            $letter = $letter_og->letter;
            $dt = Carbon::now();
            $dt->toDayDateTimeString();
...
//                $exp_reason_letter_record = new DisputeLetterRecord();
//                $exp_reason_letter_record->dispute_letter_id = $exp_reason_letter->id;
//                $exp_reason_letter_record->dispute_id = $value;
//                $exp_reason_letter_record->save();

                $get_dispute = Dispute::find($value);
                $get_dispute = $dispute->accountHistory->accountTitle->accounts->where('bureau_id',2)->first();
                $account_detail = 'Account No: '.$get_dispute->account. ' <br> '.' Reason: '. ucwords($letter_og->title) .' <br> ';
                $accounts  .= $account_detail;
